...
#include "Simulated_Annealing_3.h"
...
double Simulated_Annealing_3(vector< vector <double> >& route_time, vector<node_ID>& current_node_path){
    ...
    TSP_Instance_3 tsp_instance;
 
    
    tsp_instance.createNew_3(route_time, current_node_path.size(), current_node_path);
    ...
    ChainReverseMove_3 move1;
    SwapCityMove_3 move2;
    RotateCityMove_3 move3;
 
    
    //Add these strategies into optimize plan "moves" 
    optimizer.add_move_3(&move1);
    
    optimizer.add_move_3(&move2);
    
    optimizer.add_move_3(&move3);
    
    
    //Set up the parameter for annealing, including initial temperature, 
    //end temperature and delta, which is constant
    Geometric_Cooling_Action_3 action(20000, 0, 0.6);
    ...
    optimizer.inner_loop = 4000;
        
    //Start optimization
    optimizer.Optimize_3(tsp_instance, current_node_path);   
...
double Optimizer_3::Optimize_3(const TSP_Instance_3 & instance, std::vector<node_ID> & result) {
...
//    globalVar.Tabu_List_Corresponding_Value.clear();
    ...
    int n = static_cast<int>(instance.get_cities_3().size());
...
//    cout << endl;
    ...
    config.energy = calculate_tour_length_3(config.state);
    ...
    config.temp = cooling_action->initial_temp_3();
    ...
    Optimizer_3::Move_Service_3* service = new Optimizer_3::Move_Service_3(n);   
    
    
    for(size_t i = 0; i < moves.size(); i++){
        
        moves[i]->set_Move_Service_3(service);
...
//    double Tabu_best_energy;
    ...
    for(config.out_loop = 0; config.out_loop < outer_loop; config.out_loop++){
        ...
        config.temp = cooling_action-> next_temp_3(config);
...
//    }
        ...
        /*********************************************************************/
            ...
            const double delta = moves[m]->propose_3(SA_proposal);
