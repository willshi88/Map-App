...
#define MWIDTH          1 /* Width of menu window */
...
static void zoom_navi(void (*drawscreen) (void));
...
//a similar but my own zoom in functions for my own button

void my_zoom_in(void (*drawscreen) (void)) {
    zoom_in(drawscreen);
}

void my_zoom_out(void (*drawscreen) (void)) {
    //if (!LOD_area_test(1000000)) return;
    zoom_out(drawscreen);
}

void my_zoom_fit(void (*drawscreen) (void)) {
    zoom_fit(drawscreen);
}

void my_zoom_navi(void (*drawscreen) (void)) {
    zoom_navi(drawscreen);
}
...
void my_exit(void (*drawscreen) (void)) {
    quit(drawscreen);
}
...
/* Sets the current graphics context color to cindex if it differs from the old colour */
...
//Harry's function to draw from the left

void draw_alined_text(float xc, float yc, const std::string& str_text, float boundx, float boundy) {
    // Need a C-string to call the low-level (X11 or win32) apis.
    const char* text = str_text.c_str();
    int text_byte_length = strlen(text);
    float angle = PI * gl_state.currentfontrotation / 180.;
...
handle_zoom_in(float x, float y, void (*drawscreen) (void)) {
    ...
    //does not allow the user to zoom in infinitely
    if (LOD_area_test(globalVar.zoomScale / 3)) return;
...
handle_zoom_out(float x, float y, void (*drawscreen) (void)) {
    //does not allow the user to zoom out infinitely
    if (!LOD_area_test(1000000)) return;
...
static void
zoom_navi(void (*drawscreen) (void)) {
    trans_coord.xleft = trans_coord.init_xleft;
    trans_coord.xright = trans_coord.init_xright;
    trans_coord.ytop = trans_coord.init_ytop;
    trans_coord.ybot = trans_coord.init_ybot;
    ...
    float x1 = globalVar.siArray[getStreetSegmentInfo(
    globalVar.naviRoute_seg[0]).from].x - globalVar.x_min;
    float y1 = (globalVar.siArray[getStreetSegmentInfo(
    globalVar.naviRoute_seg[0]).from].y - globalVar.y_max);
    float x2 = globalVar.siArray[getStreetSegmentInfo(
    globalVar.naviRoute_seg[globalVar.naviRoute_seg.size()-1]).to].x - globalVar.x_min;
    float y2 = globalVar.siArray[getStreetSegmentInfo(
    globalVar.naviRoute_seg[globalVar.naviRoute_seg.size()-1]).to].y - globalVar.y_max;
    double x_left = min(x1, x2) * globalVar.x_scale;
    double x_right = max(x1, x2) * globalVar.x_scale;
    double y_bot = min(y1, y2) * globalVar.y_scale;
    double y_top = max(y1, y2) * globalVar.y_scale;


    trans_coord.xleft = (x_left);
    trans_coord.xright = (x_right);
    trans_coord.ytop = (y_top) + 25;
    trans_coord.ybot = (y_bot) - 25;
    update_transform();
    drawscreen();
}
            ...
            case ButtonPress:
                ...
                if (report.xbutton.window == x11_state.toplevel) {
                    ...
                    switch (report.xbutton.button) {
                        case Button1:
                            if (globalVar.error) break;
                            panning_on(report.xbutton.x, report.xbutton.y);
                            break; /* Left mouse click; pass back to client program */
                        ...
                        case Button5: /* Scroll wheel rotated backward; screen does zoom_out */
                            ...
                            //Harry's change to the function
                            //where it can only zoom to a certain level
                            //my_zoom_out(drawscreen);
                            break;
                        default:
