...
double time_difference_chain_reverse_3(vector< vector<double> > & route_time, std::vector<node_ID> & state, node_ID i, node_ID j);

double time_difference_swap_3(vector< vector<double> > & route_time, std::vector<node_ID> & state, node_ID i, node_ID j);

double time_difference_rotate_3(vector< vector<double> > & route_time, std::vector<node_ID> & state, node_ID i, node_ID j, node_ID k);

double Simulated_Annealing_3(vector< vector <double> >& route_time, vector<node_ID>& current_node_path);
...
public:   
    ...
    RanGen_3(unsigned num_of_cities):
...
public:   
    ...
    void createNew_3(vector< vector<double> > & route_time_, 
    ...
    const std:: vector<node_ID> & get_cities_3() const {
        return cities;
    }
...
double calculate_tour_length_3(const std::vector<node_ID> & tour);
...
public:    
    ...
    friend class SwapCityMove_3;
    
    
    class Config_3{
    public:
        //constructor innitialization
        Config_3(): temp(0), out_loop(0), inner_loop(0), energy(0),best_energy(0), 
                finish(false){}
    ...
    class Cooling_Action_3{
        
    public:
        virtual double next_temp_3(const Config_3 & config) const = 0;
        
        virtual double initial_temp_3() const = 0;
    ...
    public:
        ...
        Move_Service_3(unsigned num_of_cities):
        generator({std::random_device{}()}), distribution(0, num_of_cities -1){
    ...
    public:
        ...
        virtual double propose_3 (std::vector<node_ID> & state) const = 0;

        
        //set the random service
        void set_Move_Service_3(Move_Service_3* _service){
            service = _service;
        }
    ...
    Optimizer_3() :cooling_action(0), 
            ...
            //The cooling action
            Cooling_Action_3* cooling_action;
            ...
            double Optimize_3(const TSP_Instance_3 & instance, std::vector<node_ID> & result);             
            
            void add_move_3(Move_3* move){
...
/***************Tabu search part for optimizing path selection*****************/            
            ...
            int return_Best_Solution_Index_in_Tabu_List_3();
            ...
            void routine_Replacement_of_Tabu_List_and_Energy_3(vector<int> & current_solution, double & current_energy);
            
            
            
            
            double Tabu_Generate_the_best_neighbour_3(vector<int> & current_solution, Move_3* & swap, Config_3 & config,  vector<int> & proposed_solution);
            ...
            void aspiration_Rule_Selection_3(vector<int> & current_solution, double & current_energy);
...
//bool if_in_Tabu_List_3(vector<int> & current_solution);





class Geometric_Cooling_Action_3 : public Optimizer_3::Cooling_Action_3 {
    

public:    
       Geometric_Cooling_Action_3(double initial_temp_, double end_temp_, double alpha_) :
        ...
        virtual double next_temp_3(const Optimizer_3::Config_3 & config) const {
            
            return std::max(config.temp * alpha, end_temp);
        }

        virtual double initial_temp_3() const{
...
class ChainReverseMove_3 : public Optimizer_3::Move_3{
    
    
    //strategy to compute a random neighbour
    
    virtual double propose_3(std::vector<node_ID> & state) const {
        ...
        while(!legal){
            ...
            random1 = service -> sampling_3();
            random2 = service -> sampling_3();
...
//            
//            if (if_in_Tabu_List_3(potential_solution) == true)
//                legal == false;
//                
//                
//                
        ...
        double travel_distance_temp = time_difference_chain_reverse_3(globalVar.travel_distance, state, random1, random2);              
        //********************Exclude depot cases****************************
...
class SwapCityMove_3 : public Optimizer_3::Move_3 {
...
//            if (if_in_Tabu_List_3(potential_solution) != true)
        ...
        double travel_distance_temp = time_difference_swap_3(globalVar.travel_distance, state, random1, random2);
...
class RotateCityMove_3 : public Optimizer_3::Move_3{
public:
    ...
    vector<int> ID_TO_INDEX(state.size(), 0);
        ...
        while(!legal){
           std::vector<unsigned> random({service->sampling_3(),service->sampling_3(),service->sampling_3()});
...
//        cout << endl;
        ...
        double travel_distance_temp = time_difference_rotate_3(globalVar.travel_distance, state, index_vector[0], index_vector[1], index_vector[2]);
